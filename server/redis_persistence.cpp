...
bool redis_persistence::start()
{
	...
	// asio::ip::tcp::resolver resolver(socket_.get_io_context());
        asio::ip::tcp::resolver resolver(*executor.target<asio::io_context>());
...
{
    AUTOCRAFT_LOG("REDIS", "LOADING ALL NON PLAYABLE ENTITIES");

    std::vector<std::string> keys = helper_->keys("npe:*");

    if (keys.empty())
    {
        AUTOCRAFT_LOG("REDIS", "NOTHING TO LOAD");
        return entities();
    }
...
{
    AUTOCRAFT_LOG("REDIS", "LOAD PLAYER " << name);
    ...
    helper_->async_get("playername:" + name, [this_p, name_p, handler_p](bool success, std::string && value) {
        ...
        if (!success)
        {
            AUTOCRAFT_LOG("REDIS", "PLAYER DOES NOT EXIST, CREATING IT");
            // We create a default entity
            auto new_ent = std::make_shared<player>();

            AUTOCRAFT_LOG("REDIS", "STORING IT IN player:<id> table");
            ...
            this_p->helper_->async_set("player:" + std::to_string(new_ent->id()), new_ent->save().dump(), [this_p, name_p, handler_p, new_ent]() {
                ...
                AUTOCRAFT_LOG("REDIS", "STORING ITS ID IN playername:<name> table");
                ...
                this_p->helper_->async_set("playername:" + *name_p, std::to_string(new_ent->id()), [this_p, handler_p, new_ent]() {
                    ...
                    AUTOCRAFT_LOG("REDIS", "LOAD PLAYER DONE, CALLING HANDLER");
                    (*handler_p)(new_ent);
                });
        ...
        {
            AUTOCRAFT_LOG("REDIS", "PLAYER EXISTS, LOADING IT");
