...
void server::shutdown()
{
    ...
    AUTOCRAFT_LOG("SHUTDOWN", "Stopping game loop thread");
    *stop_ = true;
    game_cycle_timer_.cancel();

    AUTOCRAFT_LOG("SHUTDOWN", "Cancelling server accept");
    acceptor_.cancel();

    for (auto conn : player_conns_)
    {
        AUTOCRAFT_LOG("SHUTDOWN", "Closing connection " << conn->addr_str);
        conn->close();
    }
    player_conns_.clear();
    ...
    AUTOCRAFT_LOG("SHUTDOWN", "Closing server socket");
    acceptor_.close();

    AUTOCRAFT_LOG("SHUTDOWN", "Stopping persistence instance");
    persistence_->stop();
}
...
{
    AUTOCRAFT_LOG("STARTUP", "LOADING WORLD");
    ...
    /*AUTOCRAFT_LOG("STARTUP", "LOADED " << static_cast<entity_container<stationnary_entity>>(entities_).size() << " STATIONNARY ENTITIES");
    AUTOCRAFT_LOG("STARTUP", "LOADED " << static_cast<entity_container<npc>>(entities_).size() << " CHARACTER ENTITIES");*/
}
...
void server::start_game()
{
    *stop_ = false;
    ...
    AUTOCRAFT_LOG("STARTUP", "FIRST GAME CYCLE IN "
...
void server::start_network()
{
    ...
    AUTOCRAFT_LOG("STARTUP", "LISTENNING FOR PLAYER CONNECTION");
}
...
void server::game_cycle(boost::system::error_code const& error, unsigned int nb_ticks)
{
    ...
    // if (*stop_)
    // {
    //     AUTOCRAFT_LOG("GAME LOOP", "STOPPED");
    //     return;
    // }

    // double delta = (std::chrono::duration_cast<delta_duration>(tick_duration_) * nb_ticks).count();

    //AUTOCRAFT_LOG("GAME CYCLE", nb_ticks << " TICKS");

    auto it = std::remove_if(conns_.begin(), conns_.end(), [] (std::shared_ptr<autocraft::connection> conn){
    ...
    //         AUTOCRAFT_LOG("GAME LOOP", "Removing id " << id << " from entities");

    //         entities_.erase(id);
    //     }
    //     AUTOCRAFT_LOG("GAME LOOP", "Removing conn " << (*it)->addr_str << " from connections");
    //     conns_.erase(it);
    // }
    ...
    //             else
    //                 AUTOCRAFT_LOG("GAME LOOP", "ERROR during patches application: unknown value: " << p.what);
    //         }
    //         catch (const bad_any_cast& e) {
    //             AUTOCRAFT_LOG("GAME LOOP", "ERROR during patches application: " << e.what() << ". Conn: " << c->addr_str << ", entity: " << c->player_entity()->id() << ", value name: " << p.what);
    //         }
    //     }
    // }
    ...
    // }
     ...
     if (nb_ticks > 1)
         AUTOCRAFT_LOG("GAME LOOP", "RETARD OF " << nb_ticks - 1 << " TICK" << (nb_ticks - 1 > 1 ? "S" : ""));
...
void server::on_accept(const boost::system::error_code& ec) noexcept
{
    ...
    {
        if (ec.value() == 995)
            AUTOCRAFT_LOG("SERVER ACCEPT", "INFO: operation aborted");
        else
            AUTOCRAFT_LOG("SERVER ACCEPT", "ERROR " << ec.value() << ": " << ec.message());
    ...
    try {
        ...
        AUTOCRAFT_LOG("SERVER ACCEPT", "New connection from: " << endpoint);
    }
    catch (...) {
        AUTOCRAFT_LOG("SERVER ACCEPT", "ERROR while creating new player entity, new connection or while starting it");
        boost::system::error_code ec;
        new_client_socket_.close(ec);
    }

    auto new_conn = std::make_shared<connection>(std::move(new_client_socket_), io_context_, shared_from_this());
