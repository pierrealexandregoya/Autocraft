    ...
    , msg_(msg)
{
    ...
    AUTOCRAFT_LOG(msg_, std::chrono::duration_cast<readable_duration>(in_time_ - start_time).count());
}
...
scope_time::~scope_time()
{
    ...
    AUTOCRAFT_LOG(msg_, std::chrono::duration_cast<readable_duration>(out_time - start_time).count() << " (+" << std::chrono::duration_cast<readable_duration>(out_time - in_time_).count() << ")");
}
